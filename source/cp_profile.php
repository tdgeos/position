...
if($_GET['op'] == 'base') {
	...
	while ($value = $_SGLOBAL['db']->fetch_array($query)) {
		...
		$array=$_SGLOBAL['db']->query($sql);
		while($asql=$_SGLOBAL['db']->fetch_array($array)){
			$ye[]=$asql;
		}
		$count=count($ye);
		//大学  
		...
		//默认学院
		$sql="select * from `".UC_TPRE."info_school` a inner join `".UC_TPRE."info_college` b on a.schoID=b.schoID where a.schoName='".$schoName[0]."'";
		$re=$_SGLOBAL['db']->query($sql);
		while($array= $_SGLOBAL['db']->fetch_array($re)){
			$collName[]=$array['collName'];
		}
		//默认系所
		$sql="select * from  `".UC_TPRE."info_school` a inner join `".UC_TPRE."info_college` b on a.schoID=b.schoID inner join `".UC_TPRE."info_major` c on c.schoID=b.schoID and c.collID=b.collID  where schoName='".$schoName[0]."' and b.collName='".$collName[0]."'";
		$re=$_SGLOBAL['db']->query($sql);
		while($array= $_SGLOBAL['db']->fetch_array($re)){
			$majoName[]=$array['majoName'];
		}
		//默认班级
		$sql="select * from  `".UC_TPRE."info_school` a inner join `".UC_TPRE."info_college` b on a.schoID=b.schoID inner join `".UC_TPRE."info_major` c on c.schoID=b.schoID and c.collID=b.collID inner join `".UC_TPRE."info_class` d on d.schoID=c.schoID and d.collID=c.collID  and d.majoID=c.majoID  where a.schoName='".$schoName[0]."' and b.collName='".$collName[0]."' and c.majoName='".$majoName[0]."'";
		$re=$_SGLOBAL['db']->query($sql);
		while($array= $_SGLOBAL['db']->fetch_array($re)){
	...
	if(submitcheck('profilesubmit') || submitcheck('nextsubmit')) {
		...
		$tim = $_SGLOBAL['timestamp'];
		if('学校名称' == $scho){
			echo "<script>alert('请填写学校名称');location.href='cp.php?ac=profile&op=edu'</script>";die;	
		}elseif('学院名称' == $coll){
			echo "<script>alert('请填写学院名称');location.href='cp.php?ac=profile&op=edu'</script>";die;	
		}elseif('专业名称' == $majo){
			echo "<script>alert('请填写专业名称');location.href='cp.php?ac=profile&op=edu'</script>";die;	
		}elseif('班级名称' == $class){
			echo "<script>alert('请填写班级名称');location.href='cp.php?ac=profile&op=edu'</script>";die;
		}elseif('入学年份' == $startyear){
			echo "<script>alert('请填写入学年份');location.href='cp.php?ac=profile&op=edu'</script>";die;
		}
		$sq="SELECT * FROM ".UC_MEMBER."schoolinfo";
		$re1=$_SGLOBAL['db']->query($sq);
			while($array= $_SGLOBAL['db']->fetch_array($re1)){
					$auid[]=$array['uid'];
			}
		if(!in_array($uid,$auid))
		{
				$sql="INSERT ".UC_MEMBER."schoolinfo (uid,schoName,schoRole,collName,collRole,majoName,majoRole,clasName,clasRole,schoolyear,studenttype,studentnumber,updatetime) VALUES
		('$uid', '$scho', 0, '$coll', 0, '$majo', 0, '$class', 0, $startyear, 0, '$tim', 0)";
				$is=$_SGLOBAL['db']->query($sql);
				if($is){
					echo "<script>location.href='cp.php?ac=profile&op=edu'</script>";	
				}
			}
	...
	if($_GET['subop'] == 'delete')
	{
		...
		$sql="DELETE FROM ".UC_MEMBER."schoolinfo WHERE uid='$infoid'";
		if($infoid) {
			$re=$_SGLOBAL['db']->query($sql);
			if($re){
				echo "<script>location.href='cp.php?ac=profile&op=edu'</script>";	
	...
	if($_GET['subop'] == 'delete') {
		$infoid = intval($_GET['infoid']);
		if($infoid) {
			$_SGLOBAL['db']->query("DELETE FROM ".UC_MEMBER('schoolinfo')." WHERE infoid='$infoid'");
		}
	}
