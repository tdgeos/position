...
function showmessage($msgkey, $url_forward='', $second=1, $values=array()) {
	...
	//echo $message;
	$message = $_SGLOBAL['msglang'][$msgkey];
	//手机
	if($_SGLOBAL['mobile']) {
...
//添加数据
function sinserttable($tablename, $insertsqlarr, $returnid=0, $replace = false, $silent=0) {
	global $_SGLOBAL;
...
//更新数据
function supdatetable($tablename, $setsqlarr, $wheresqlarr, $silent=0) {
	global $_SGLOBAL;
...
function multi($num, $perpage, $curpage, $mpurl, $ajaxdiv='', $todiv='') {
	...
	if($num > $perpage) {
		...
		if($multipage) {
			$multipage = '<em>当前共有&nbsp;'.$num.'&nbsp;条心情</em>'.$multipage;
		}
	}
	return $multipage;
}
...
function mkfeed($feed, $actors=array()) {
	...
	//icon
	if($feed['appid']) {
		$feed['icon_image'] = "image/icon/{$feed['icon']}
.gif";
	} else {
