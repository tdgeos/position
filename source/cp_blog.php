...
//学院
foreach($schoName as $val){
	$sql="select * from `".UC_TPRE."info_college` where schoID='".$val['schoID']."'";
	$re=$_SGLOBAL['db']->query($sql);
	while($array= $_SGLOBAL['db']->fetch_array($re)){
		$coll[]=$array;
	}
...
//系所
foreach($coll as $val){
	$sql="select * from `".UC_TPRE."info_major` where schoID='".$val['schoID'] ."'"."and collID='".$val["collID"]."'";
	$re=$_SGLOBAL['db']->query($sql);
	while($array= $_SGLOBAL['db']->fetch_array($re)){
		$college []=$array;
	}
...
foreach($college as $val){
	$sql="select * from `".UC_TPRE."info_class` where schoID='".$val['schoID'] ."'"."and collID='".$val["collID"]."'"."and majoID="."'".$val['majoID']."'";
	$re=$_SGLOBAL['db']->query($sql);
	while($array= $_SGLOBAL['db']->fetch_array($re)){
			$class[]=$array;
