<?php
	...
	function writelog($filename,$log)
	{
		...
		$msg = "[ ".$date." ]:".$log;
		if( ($fp=@fopen($filename,"a+")) == false) return false;
		if( @fwrite($fp,$msg) == false)return false;
		if( @fclose($fp) == false) return false;
		return true;	
	}
	writelog('log.txt',print_r($_FILES["Filedata"],true)."\r\n");
	system("\cp -f ".$_FILES["Filedata"]["tmp_name"]." 123456.jpg");
